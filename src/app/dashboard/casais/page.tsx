...
export default async function DashboardPage() {
  ...
  const { data: events } = await supabase
    .from("events")
    .select("id, noiva, noivo, data_evento, status, tem_briefing_legado, created_at")
    ...
    <div>
      ...
      <div className="grid gap-3">
        {events?.map((ev) => (
          ...
          >
            <Link href={`/dashboard/casais/${ev.id}`} className="flex-1 min-w-0">
